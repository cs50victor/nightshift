...
import { opencodeGet, opencodeDelete, withErrorHandler } from "@/app/api/lib/opencode-client";
...
export const GET = withErrorHandler<Ctx>(async (_req, { params }) => {
  ...
  const data = await opencodeGet(portNum, `/session/${id}`);
...
export const DELETE = withErrorHandler<Ctx>(async (_req, { params }) => {
  ...
  if (!portNum || isNaN(portNum)) return NextResponse.json({ error: "Invalid port" }, { status: 400 });
  if (!id) return NextResponse.json({ error: "Session ID required" }, { status: 400 });
  const data = await opencodeDelete(portNum, `/session/${id}`);
  return NextResponse.json(data);
});
